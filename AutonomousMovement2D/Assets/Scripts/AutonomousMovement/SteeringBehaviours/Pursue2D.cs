...
namespace Kensai.AutonomousMovement {
    public class Pursue2D : SteeringBehaviour2D {
        ...
        public static Vector2 GetVelocity(SteeringAgent2D agent, SteeringAgent2D evader, float turnCoefficient = 0f) {
            ...
            var toEvader = evader.rigidbody2D.position - agent.rigidbody2D.position;
            float relativeHeading = Vector2.Dot(agent.Heading, evader.transform.up);

            if ((Vector2.Dot(toEvader, agent.transform.up) > 0) &&
                relativeHeading < -0.95) { //acos(0.95) = 18 degs
                    return Seek2D.GetVelocity(agent, evader.rigidbody2D.position);
            }

            //Not considered ahead so we predict where the evader will be
            float lookAheadTime = toEvader.magnitude / (agent.MaxSpeed + evader.rigidbody2D.velocity.magnitude);
            if (turnCoefficient != 0) {
                lookAheadTime += TurnaroundTime(agent, evader.rigidbody2D.position, turnCoefficient);
            }

            return Seek2D.GetVelocity(agent, evader.rigidbody2D.position + evader.rigidbody2D.velocity * lookAheadTime);
        }
